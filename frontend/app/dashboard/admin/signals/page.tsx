...
import { ArrowDownRight, ArrowUpRight, Plus, RefreshCw, Trash2, X, Edit2 } from "lucide-react";
...
const STATUSES = ["ACTIVE","TP_HIT","SL_HIT","EXPIRED"];

const defaultForm = { pair: "", direction: "BUY", entry: "", tp: "", sl: "", strength: "80", timeframe: "H4", model: MODELS[0] };
...
export default function AdminSignalsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const res = await fetch(`${API}/api/signals`, { headers: getAuthHeaders() });
  ...
  const handleDelete = async (id: string) => {
    ...
    await fetch(`${API}/api/signals/${id}`, { method: "DELETE", headers: getAuthHeaders() });
      ...
      <div className="rounded-xl border border-white/[0.06] bg-[#0a0f1a] p-6">
        <div className="flex items-end justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-400">Publish AI signals — notifies all active subscribers instantly.</p>
          </div>
      ...
      {showForm && (
        <div className="rounded-xl border border-[#e7b949]/15 bg-[#0a0f1a] p-5">
          ...
          <div className="mt-4 flex gap-3">
            ...
            <button onClick={() => setShowForm(false)} className="rounded-xl border border-white/[0.08] px-4 py-2.5 text-sm text-slate-400 hover:text-white">Cancel</button>
          </div>
        </div>
      )}
            ...
            {signals.map((s) => (
              <div key={s.id} className="flex flex-wrap items-center gap-3 rounded-xl border border-white/[0.05] bg-white/[0.02] px-4 py-3">
                <div className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg ${s.direction === "BUY" ? "bg-emerald-500/10" : "bg-red-500/10"}`}>
                  {s.direction === "BUY" ? <ArrowUpRight className="h-4 w-4 text-emerald-400" /> : <ArrowDownRight className="h-4 w-4 text-red-400" />}
                </div>
                <div className="flex-1 min-w-[120px]">
                  <p className="text-sm font-bold text-white">{s.pair} <span className="font-mono text-xs text-slate-500">@ {s.entry}</span></p>
                ...
                {s.status === "ACTIVE" && (
                  <div className="flex gap-1">
                    <button onClick={() => { const p = prompt("Pips (TP hit):"); handleStatusUpdate(s.id, "TP_HIT", p ? parseFloat(p) : undefined); }}
                      disabled={updatingId === s.id}
                      className="rounded-lg bg-blue-500/10 px-2 py-1 text-[10px] font-bold text-blue-400 hover:bg-blue-500/20 disabled:opacity-40">TP</button>
                    <button onClick={() => { const p = prompt("Pips (SL hit):"); handleStatusUpdate(s.id, "SL_HIT", p ? parseFloat(p) : undefined); }}
                      disabled={updatingId === s.id}
                      className="rounded-lg bg-red-500/10 px-2 py-1 text-[10px] font-bold text-red-400 hover:bg-red-500/20 disabled:opacity-40">SL</button>
                  </div>
                )}
